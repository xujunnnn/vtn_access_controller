 ...
 */
package vtn_access_controller.impl.core.util;
...
public final class InstanceIdentifierUtils {
    ...
    public static byte[] toToken(byte[] key) {
        try {
     ...
     * @return
     */
   public static String buildIfName(VbridgePath path,NodeConnectorId port){
		String[] portinfo=port.getValue().split(":");
		String nodeid=portinfo[1];
		String portid=portinfo[2];
		return path.getVtn()+"_"+path.getVbridge()+"_"+nodeid+"_"+portid;
	}
   
   public static String buildIfName(VbridgePath path,MacAddress macAddress){
 		String mac=macAddress.getValue().replace(':', '_');
 		return path.getVtn()+"_"+path.getVbridge()+"_"+mac;
 	}
}
